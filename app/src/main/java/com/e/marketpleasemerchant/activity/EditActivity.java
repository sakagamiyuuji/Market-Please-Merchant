...
public class EditActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void getOldData(){
        ...
        String url = "http://210.210.154.65:4444/api/product-by-id/"+id;

        JsonObjectRequest getOldDataReq = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            edtProductDesc.setText(product.getProductDesc());
                            edtProductPrice.setText(String.valueOf(product.getProductPrice()));
                            edtProductQty.setText(String.valueOf(product.getProductQty()));

                            String urlImg = "http://210.210.154.65:4444/storage/" + product.getProductImage();
                            Toast.makeText(EditActivity.this, urlImg, Toast.LENGTH_SHORT).show();

                            if (product.getProductImage() == null){
                                Glide.with(EditActivity.this)
                                        .load(R.drawable.image_addthumb)
                                        .into(edtProductImg);
                            }
                            else{
                                Glide.with(EditActivity.this)
        ...
        VolleyApp.getInstance().addToRequestQueue(getOldDataReq, "getOldData");
    }
